...
export async function startVideoAnalysis(url: string): Promise<{ task_id: string; status: string }> {
  const { data } = await api.post<{ task_id: string; status: string }>("/video/analyze", { url });
  return data;
}

export async function getVideoAnalysisResults(taskId: string): Promise<FullAnalysisResult> {
  const { data } = await api.get<FullAnalysisResult>(`/video/results/${taskId}`);
  return data;
}

export function getVideoStreamUrl(taskId: string): string {
  return `/api/video/video/${taskId}`;
}
